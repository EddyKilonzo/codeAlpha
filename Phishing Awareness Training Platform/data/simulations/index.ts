  ...
  'introduction': [
    {
      ...
      hints: [
        'Start by examining who sent this email — does the domain after the @ actually belong to your company?',
        'Legitimate IT teams rarely give you only 2 hours to act. Ask yourself: what emotion is this deadline designed to trigger?',
        'Before trusting any button in an email, think about where it might actually take you — is "company-helpdesk.net" your company\'s real IT portal?',
      ],
      flags: [
  ...
  'types-of-phishing': [
    {
      ...
      hints: [
        'Real couriers send SMS from registered short codes (short number IDs), not from random mobile numbers like the one here.',
        'Look very carefully at the link — does the domain actually belong to the company it claims to be from?',
        'Ask yourself: would a legitimate delivery company ever ask you to pay customs fees via a link in a text message?',
      ],
      flags: [
    ...
    },
    {
      ...
      hints: [
        'Look closely at what email address the CEO is actually writing from — does it end in your company\'s domain?',
        'Notice the two social engineering tactics used together in this email — urgency and secrecy. Why would both be necessary for a legitimate request?',
        'For a wire transfer of this size, what process should always happen before any money moves, regardless of who asks?',
      ],
      flags: [
  ...
  'attacker-operations': [
    {
      ...
      hints: [
        'Read the URL in the address bar very slowly, character by character — the real domain is the part just before the first single forward slash.',
        'Think about what your organisation\'s real Microsoft login normally shows you. Does anything look missing from this page?',
        'A padlock in the address bar means the connection is encrypted — but it does NOT mean the site is legitimate. What else can you check?',
      ],
      flags: [
  ...
  'advanced-threats': [
    {
      ...
      hints: [
        'Start with the sender\'s email address — is "microsoft-authenticator-support.com" actually a Microsoft domain?',
        'Unlike a regular link that email security tools can scan, what is hidden inside a QR code image that makes it harder to detect?',
        'Think about which device you would use to scan this QR code — does that device have the same corporate security protections as your work laptop?',
      ],
      flags: [
        { id: 'f1', label: 'Sender domain is suspicious', description: '"security-team@microsoft-authenticator-support.com" is NOT a Microsoft domain. Real Microsoft emails come from @microsoft.com.', position: { top: 8, left: 2, width: 75, height: 6 }, type: 'malicious', pointValue: 25 },
        { id: 'f2', label: 'Urgency — enrollment deadline', description: '"Complete by end of today" — artificial urgency to prevent careful verification of the QR code destination.', position: { top: 30, left: 0, width: 100, height: 8 }, type: 'warning', pointValue: 15 },
        { id: 'f3', label: 'QR code bypasses email scanners', description: 'Email security gateways cannot scan QR codes — the malicious URL is hidden inside the image. This is exactly why quishing is effective.', position: { top: 45, left: 25, width: 50, height: 25 }, type: 'malicious', pointValue: 35 },
...
Microsoft Security Team`,
    ...
    },
    {
      ...
      hints: [
        'Look very carefully at the sender\'s display name — there is a word in parentheses that tells you something important about this account.',
        'If your CFO genuinely needed an emergency wire transfer, what would be the proper way to verify that request through a separate channel?',
        'Why would any legitimate financial process require total secrecy from your colleagues and management?',
      ],
      flags: [
  ...
  'case-studies': [
    {
      ...
      hints: [
        'Read the URL character by character — what is the actual domain name (the part immediately before the first single forward slash)?',
        'If you use a password manager, notice whether it offered to fill in your PayPal password on this page.',
        'Think about how you arrived at this page — did you type the address yourself, or did you follow a link in an email?',
      ],
      flags: [
  ...
  'defense-best-practices': [
    {
      ...
      hints: [
        'Look beyond the display name — expand each email\'s actual sender address and check whether the domain is the real one for that company.',
        'Which of these emails creates a sense of urgency or threatens a negative consequence if you don\'t act quickly?',
        'For the email that looks suspicious, think about where any "verify" or "click here" links might actually lead.',
      ],
      flags: [
    ...
    },
    {
      ...
      hints: [
        'A certificate error means the site\'s identity could not be verified — what does that tell you about whether you should proceed?',
        'There are two choices on this page: one is safe, one is risky. Which button label gives that away?',
        'Check the URL shown in the warning — does it match the site you actually intended to visit?',
      ],
      flags: [
